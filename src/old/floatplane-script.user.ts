...
import nws from '../scripts/nws.lib';
...
const floatplaneScript2 = () => {
	...
	const waitForElementToExist = async (
	...
	const configuringLiveVideo = () => {
		const liveStreamView = document.querySelector<HTMLElement>(
			'div.route-wrapper.livestream-view'
		);
		...
		const liveScrollWrapper =
			liveStreamView.querySelector<HTMLElement>('div.live-scroll-wrapper');
		if (liveScrollWrapper === null) return;
		liveScrollWrapper.style.flexShrink = '0';

		const playerContainer = liveScrollWrapper.querySelector<HTMLElement>(
			'div.player-container.livestream'
		);
		...
		const videoJS = streamContainer.querySelector<HTMLElement>(
	...
	const configuringLiveChat = (liveChatWrapper: HTMLElement) => {
		...
		const chatInputContainer = liveChatWrapper.querySelector<HTMLElement>(
	...
	const checkIfLive = async (list: floatplane.listResponse) => {
		const creators = list.blogPosts
			...
			.reduce<floatplane.creator[]>((acc, curr) => {
	...
	const setupCheckers = () => {
		...
		const checkers = (feed: HTMLElement) => {
			const latestFeedItem = feed.querySelector<HTMLAnchorElement>(
				'div.ReactElementGridItem > a.ReactPostTile.video'
			);
