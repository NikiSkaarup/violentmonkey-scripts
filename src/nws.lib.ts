export default (() => {
  ...
  const createHTMLElement = (
    ...
    className?: string,
    innerHTML?: string
  ): HTMLElement => createElement<HTMLElement>(tag, className, innerHTML);

  // UI elements
  const ui = {
    configurationWindowContainer: createHTMLElement(
      'div',
      'nws-configuration-window-container'
    ),
    configurationWindow: createHTMLElement('div', 'nws-configuration-window'),
    header: createHTMLElement('h1', 'nws-header'),
    configurationWindowSubContainer: createHTMLElement(
      'div',
      'nws-sub-container nws-sub-container-extra'
    ),
  ...
  const initConfigUI = () => {
    ...
    const subContainerContent = createHTMLElement(
      'div',
    ...
    const debuggingLabel = createElement<HTMLLabelElement>(
      'label',
      'nws-label'
    );
    ...
    const debuggingCheckboxContainer = createHTMLElement(
      'div',
      'nws-checkbox-container'
    );
    ...
    footerHeading.innerText =
      'These settings are stored in each scripts own values';
  ...
  const tabSwitch = (event: KeyboardEvent) => {
    ...
    const elements =
      ui.configurationWindowContainer.querySelectorAll<HTMLElement>(
        focusableSelector
      );
  ...
  const noModifier = (e: KeyboardEvent) =>
    !(e.ctrlKey || e.altKey || e.shiftKey);
  const shiftModifier = (e: KeyboardEvent) =>
    e.shiftKey && !(e.ctrlKey || e.altKey);
  const ctrlModifier = (e: KeyboardEvent) =>
    e.ctrlKey && !(e.altKey || e.shiftKey);
  const altModifier = (e: KeyboardEvent) =>
  ...
  const configOpenShortcuts: shortcutCallback = (
    event: KeyboardEvent
  ): boolean => {
    switch (true) {
  ...
  const configClosedShortcuts: shortcutCallback = (
    event: KeyboardEvent
  ): boolean => {
    switch (true) {
  ...
  const filterRegisteredShortcut = (
    shortcutType: shortcutType
  ): registeredShortcut[] =>
    registeredKeyUpShortCuts.filter(
